...
void myprintf(const int8 *format, ...)
{
    ...
    while (*format)
    {
        ...
        if (ret == '%')
        {
            switch (*++format)
            {
                ...
                case 'i':// 有符号十进制整数
                {
                    ...
                    if(ival<0)  uart_putchar(UART_2, '-');
                    printf_reverse_order(vstr,vlen);
                    uart_putbuff(UART_2, (uint8 *)vstr,vlen);
                }break;
                ...
                case 'p':// 以16进制形式输出指针
                {
                    ...
                    uart_putbuff(UART_2, (uint8 *)vstr,8);
        ...
        {
            uart_putchar(UART_2, (int8)(*format));
        }
        format++;
    }
    va_end(arg);
