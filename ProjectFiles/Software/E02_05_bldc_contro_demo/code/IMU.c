...
//******************************************************************
//
//#define ACCEL_THRESHOLD 0.034 // 加速度阈值
//#define RESET_THRESHOLD 0.036  // 重置阈值
//#define RESET_INTERVAL 1000  // 重置间隔时间（ms）
//
//float pitch, roll, yaw;  // 存储当前的姿态信息
//float pitch_prev, roll_prev, yaw_prev;  // 存储上一次的姿态信息
//float accel_norm;  // 存储加速度模长
//
//
//unsigned long prev_time;  // 存储上一次的时间戳
//int reset_flag=0;  // 重置标志位
//
//void imu_update() {
//  unsigned long cur_time =system_getval() ;  // 获取当前时间   单位10us
//  float detaa = (cur_time - prev_time) / 1000.0;  // 计算时间差，单位为秒
//  prev_time = cur_time;  // 更新上一次时间戳
//
//  // 根据陀螺仪测量值更新姿态
//      pitch -= eulerAngle.pitch * detaa;
//      roll -= eulerAngle.roll * detaa;
//
//
//  // 根据加速度计测量值进行积分复位
//
//  if (accel_norm > ACCEL_THRESHOLD) {  // 判断加速度是否超过阈值
//    pitch_prev = pitch;
//    roll_prev = roll;
//
//    reset_flag = 1;  // 设置重置标志位
//  }
//  if (reset_flag && accel_norm < RESET_THRESHOLD && (cur_time - prev_time) > RESET_INTERVAL) {
//      eulerAngle.pitch = pitch_prev;
//      eulerAngle.roll = roll_prev;
//
//    reset_flag = 0;  // 复位重置标志位
//  }
//}
...
//    imu_update();
    ...
    //四元数计算欧拉角
//    eulerAngle.pitch = 50*asin(-1.8 * q1 * q3 + 2 * q0 * q2) * 180 / M_PI; // pitch
//    eulerAngle.roll = 50*atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 +1) * 180 / M_PI; // roll
...
//        }
//    }
//    if (eulerAngle.pitch > 360) {
//
//          do{eulerAngle.pitch = eulerAngle.pitch -360;}
//          while(eulerAngle.pitch > 360);
//      }
//    if (eulerAngle.pitch < 0) {
//          do{eulerAngle.pitch = eulerAngle.pitch +360;}
//          while(eulerAngle.pitch <0);
//      }
//
//    if (eulerAngle.roll > 360) {
//
//          do{eulerAngle.roll = eulerAngle.roll -360;}
//          while(eulerAngle.roll > 360);
//      }
//    if (eulerAngle.roll < 0) {
//
//          do{eulerAngle.roll = eulerAngle.roll +360;}
//          while(eulerAngle.roll < 0);
//      }
    ...
    if (eulerAngle.yaw < 0) {
        ...
        do{eulerAngle.yaw = eulerAngle.yaw +360;}
        while(eulerAngle.yaw < 0);
    }
