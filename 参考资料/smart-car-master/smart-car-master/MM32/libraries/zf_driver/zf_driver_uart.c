...
// @param       len             发送长度
// @return      void
// Sample usage:                uart_write_byte_array(UART_1,&a[0],5);
//-------------------------------------------------------------------------------------------------------------------
...
// @param       *str            要发送的字符串地址
// @return      void
// Sample usage:                uart_write_string(UART_1,"i lvoe you"); 
//-------------------------------------------------------------------------------------------------------------------
...
uint8 uart_read_byte (uart_index_enum uart_n)
{
    ...
    return (uint8_t)uart_index[uart_n]->RDR;
}
...
uint8 uart_query_byte (uart_index_enum uart_n, uint8 *dat)
{
    ...
    {
        *dat = (uint8_t)uart_index[uart_n]->RDR;                                // 存储一个数据
        return 1;
    }
    return 0;
}
...
void uart_init(uart_index_enum uart_n, uint32 baud, uart_tx_pin_enum tx_pin, uart_rx_pin_enum rx_pin)
{
    ...
    {
        case UART_1:
            RCC->APB2ENR |= ((uint32_t)0x00000001 << 4);                        // 使能 UART1 时钟
            RCC->APB2RSTR |= ((uint32_t)0x00000001 << 4);                       // 复位 UART1
            RCC->APB2RSTR &= ~((uint32_t)0x00000001 << 4);                      // 取消复位 UART1
            break;
        case UART_2:
            RCC->APB1ENR |= ((uint32_t)0x00000001 << 17);                       // 使能 UART2 时钟
            RCC->APB1RSTR |= ((uint32_t)0x00000001 << 17);                      // 复位 UART2
            RCC->APB1RSTR &= ~((uint32_t)0x00000001 << 17);                     // 完成复位 UART2
            break;
        case UART_3:
            RCC->APB1ENR |= ((uint32_t)0x00000001 << 18);                       // 使能 UART3 时钟
            RCC->APB1RSTR |= ((uint32_t)0x00000001 << 18);                      // 复位 UART3
            RCC->APB1RSTR &= ~((uint32_t)0x00000001 << 18);                     // 完成复位 UART3
            break;
        case UART_4:
            RCC->APB1ENR |= ((uint32_t)0x00000001 << 19);                       // 使能 UART4 时钟
            RCC->APB1RSTR |= ((uint32_t)0x00000001 << 19);                      // 复位 UART4
            RCC->APB1RSTR &= ~((uint32_t)0x00000001 << 19);                     // 完成复位 UART4
            break;
        case UART_5:
            RCC->APB1ENR |= ((uint32_t)0x00000001 << 20);                       // 使能 UART5 时钟
            RCC->APB1RSTR |= ((uint32_t)0x00000001 << 20);                      // 复位 UART5
            RCC->APB1RSTR &= ~((uint32_t)0x00000001 << 20);                     // 完成复位 UART5
            break;
        case UART_6:
            RCC->APB2ENR |= ((uint32_t)0x00000001 << 5);                        // 使能 UART6 时钟
            RCC->APB2RSTR |= ((uint32_t)0x00000001 << 5);                       // 复位 UART6
            RCC->APB2RSTR &= ~((uint32_t)0x00000001 << 5);                      // 完成复位 UART6
            break;
        case UART_7:
            RCC->APB1ENR |= ((uint32_t)0x00000001 << 30);                       // 使能 UART7 时钟
            RCC->APB1RSTR |= ((uint32_t)0x00000001 << 30);                      // 复位 UART7
            RCC->APB1RSTR &= ~((uint32_t)0x00000001 << 30);                     // 完成复位 UART7
            break;
        case UART_8:
            RCC->APB1ENR |= ((uint32_t)0x00000001 << 31);                       // 使能 UART8 时钟
            RCC->APB1RSTR |= ((uint32_t)0x00000001 << 31);                      // 复位 UART8
            RCC->APB1RSTR &= ~((uint32_t)0x00000001 << 31);                     // 完成复位 UART8
            break;
    }
