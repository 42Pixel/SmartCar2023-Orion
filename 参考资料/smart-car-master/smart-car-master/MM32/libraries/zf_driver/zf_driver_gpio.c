...
{
    if(gpio_group[(pin&0xE0)>>5]->IDR & (((uint16_t)0x0001) << (pin&0x1F)))     // 读取引脚值
        return 1;
    return 0;
}
...
{
    if(gpio_group[(pin&0xE0)>>5]->IDR & (((uint16_t)0x0001) << (pin&0x1F)))
        gpio_group[(pin&0xE0)>>5]->BRR |= (((uint16_t)0x0001) << (pin&0x1F));   // 输出低电平
    else
        gpio_group[(pin&0xE0)>>5]->BSRR |= (((uint16_t)0x0001) << (pin&0x1F));  // 输出高电平
}
...
void gpio_set_dir (gpio_pin_enum pin, gpio_dir_enum dir, gpio_mode_enum mode)
{
    ...
    if(0x08 > io_pin)                                                           // 低8位引脚
    {
        ...
        temp |= (((uint32_t)dir|(uint32_t)(mode&0x0C)) << (io_pin*4));          // 写入对应模式
    ...
    else                                                                        // 高8位引脚
    {
        ...
        temp |= (((uint32_t)dir|(uint32_t)(mode&0x0C)) << ((io_pin-8)*4));      // 写入对应模式
...
void gpio_init (gpio_pin_enum pin, gpio_dir_enum dir, const uint8 dat, gpio_mode_enum mode)
{
    ...
    if(0x08 > io_pin)                                                           // 低8位引脚
    {
        ...
        temp |= ((uint32_t)15 << (io_pin*4));                                   // AF 复用设置 15 保留
    ...
    else                                                                        // 高8位引脚
    {
        ...
        temp |= ((uint32_t)15 << ((io_pin-8)*4));                               // AF 复用设置 15 保留
...
void afio_init (gpio_pin_enum pin, gpio_dir_enum dir, gpio_af_enum af, gpio_mode_enum mode)
{
    ...
    if(0x08 > io_pin)                                                           // 低8位引脚
    {
        temp = gpio_group[io_group]->AFRL;
        temp &= ~(0x0000000F << (io_pin*4));                                    // 清空 AF 复用
        temp |= ((uint32_t)af << (io_pin*4));                                   // AF 复用设置对应功能
        gpio_group[io_group]->AFRL = temp;
    ...
    else                                                                        // 高8位引脚
    {
        temp = gpio_group[io_group]->AFRH;
        temp &= ~(0x0000000F << ((io_pin-8)*4));                                // 清空 AF 复用
        temp |= ((uint32_t)af << ((io_pin-8)*4));                               // AF 复用设置对应功能
        gpio_group[io_group]->AFRH = temp;
