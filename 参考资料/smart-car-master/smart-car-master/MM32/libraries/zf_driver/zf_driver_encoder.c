    ...
    switch(encoder_n)
    {
        case TIM1_ENCOEDER:
            if(0xFF == encoder_dir_pin[encoder_n])                              // 正交解码模式
                return (int16_t)TIM1->CNT/4;                                    // 返回 int16 的计数值 4096/4=1024
            else                                                                // 否则是方向编码器
                if(gpio_get_level((gpio_pin_enum)encoder_dir_pin[encoder_n]))   // 方向引脚高电平
                    return (uint16_t)TIM1->CNT;                                 // 返回正数值
                else                                                            // 方向引脚低电平
                    return -((uint16_t)TIM1->CNT);                              // 返回负数值
        case TIM2_ENCOEDER:
            if(0xFF == encoder_dir_pin[encoder_n])                              // 正交解码模式
                return (int16_t)TIM2->CNT/4;                                    // 返回 int16 的计数值 4096/4=1024
            else                                                                // 否则是方向编码器
                if(gpio_get_level((gpio_pin_enum)encoder_dir_pin[encoder_n]))   // 方向引脚高电平
                    return (uint16_t)TIM2->CNT;                                 // 返回正数值
                else                                                            // 方向引脚低电平
                    return -((uint16_t)TIM2->CNT);                              // 返回负数值
        case TIM3_ENCOEDER:
            if(0xFF == encoder_dir_pin[encoder_n])                              // 正交解码模式
                return (int16_t)TIM3->CNT/4;                                    // 返回 int16 的计数值 4096/4=1024
            else                                                                // 否则是方向编码器
                if(gpio_get_level((gpio_pin_enum)encoder_dir_pin[encoder_n]))   // 方向引脚高电平
                    return (uint16_t)TIM3->CNT;                                 // 返回正数值
                else                                                            // 方向引脚低电平
                    return -((uint16_t)TIM3->CNT);                              // 返回负数值
        case TIM4_ENCOEDER:
            if(0xFF == encoder_dir_pin[encoder_n])                              // 正交解码模式
                return (int16_t)TIM4->CNT/4;                                    // 返回 int16 的计数值 4096/4=1024
            else                                                                // 否则是方向编码器
                if(gpio_get_level((gpio_pin_enum)encoder_dir_pin[encoder_n]))   // 方向引脚高电平
                    return (uint16_t)TIM4->CNT;                                 // 返回正数值
                else                                                            // 方向引脚低电平
                    return -((uint16_t)TIM4->CNT);                              // 返回负数值
        case TIM5_ENCOEDER:
            if(0xFF == encoder_dir_pin[encoder_n])                              // 正交解码模式
                return (int16_t)TIM5->CNT/4;                                    // 返回 int16 的计数值 4096/4=1024
            else                                                                // 否则是方向编码器
                if(gpio_get_level((gpio_pin_enum)encoder_dir_pin[encoder_n]))   // 方向引脚高电平
                    return (uint16_t)TIM5->CNT;                                 // 返回正数值
                else                                                            // 方向引脚低电平
                    return -((uint16_t)TIM5->CNT);                              // 返回负数值
        case TIM8_ENCOEDER:
            if(0xFF == encoder_dir_pin[encoder_n])                              // 正交解码模式
                return (int16_t)TIM8->CNT/4;                                    // 返回 int16 的计数值 4096/4=1024
            else                                                                // 否则是方向编码器
                if(gpio_get_level((gpio_pin_enum)encoder_dir_pin[encoder_n]))   // 方向引脚高电平
                    return (uint16_t)TIM8->CNT;                                 // 返回正数值
                else                                                            // 方向引脚低电平
                    return -((uint16_t)TIM8->CNT);                              // 返回负数值
        default: return 0;
    }
}
