...
static void clock_set_freq (uint32 clock)
{
    ...
    if (0x00000001 == hse_status)
    {
        ...
        else
        {
            ...
            RCC->PLLCFGR |= ((uint32_t)(SYSTEM_CLOCK_XTAL/temp_value-1)<<8);    // 计算并写入 PLL 分频系数
            RCC->PLLCFGR |= ((uint32_t)(clock/temp_value-1)<<16);               // 计算并写入 PLL 倍频系数
...
// @param       clock           时钟频率 推荐使用 zf_common_clock.h 中 system_clock_enum 定义的选项
// @return      void
// Sample usage:                clock_init(SYSTEM_CLOCK_96M);                   // 初始化核心时钟为 96MHz
//-------------------------------------------------------------------------------------------------------------------
