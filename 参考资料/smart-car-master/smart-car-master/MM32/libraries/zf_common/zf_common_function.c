    ...
    while (*format)
    {
        ...
        if ('%' == ret)
        {
            switch (*++format)
            {
                ...
                case 'c':// 一个字符
                    {
                        int8 ch = (int8_t)va_arg(arg, uint32_t);
                        *buff = ch;
                        buff++;
                        buff_len++;
                    }
                ...
                case 'i':// 有符号十进制整数
                    {
                        int8 vstr[33];
                        int32 ival = (int32_t)va_arg(arg, int32_t);
                        uint8 vlen = number_conversion_ascii((uint32_t)ival, vstr, 1, 10);
                ...
                case 'F':// 浮点数，输出小数点后六位  不能指定输出精度
                {
                    ...
                    uint8 vlen = number_conversion_ascii((uint32_t)(int32_t)ival, vstr, 1, 10);
                ...
                case 'u':// 无符号十进制整数
                    {
                        int8 vstr[33];
                        uint32 ival = (uint32_t)va_arg(arg, uint32_t);
