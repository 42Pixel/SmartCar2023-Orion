...
#define CMU_CLK_FREQ           100000000.0f                       // CMU时钟频率
...
// 参数说明     duty            设置占空比
// 返回参数     void
// 使用示例     pwm_set_freq(ATOM0_CH7_P02_7,50, 5000);//设置频率为50HZ 占空比为百分之 5000/PWM_DUTY_MAX*100
// 备注信息     PWM_DUTY_MAX宏定义在zf_driver_pwm.h  默认为10000
//-------------------------------------------------------------------------------------------------------------------
void pwm_set_freq(pwm_channel_enum pwmch, uint32 freq, uint32 duty)
{
    IfxGtm_Atom_ToutMap *atom_channel;
    atom_channel = get_pwm_pin(pwmch);
    uint32 period = 0;

    period =  CMU_CLK_FREQ / freq;

    IfxGtm_Atom_Ch_setCompareOneShadow(&MODULE_GTM.ATOM[atom_channel->atom], atom_channel->channel, duty * period / PWM_DUTY_MAX);
...
void pwm_init (pwm_channel_enum pwmch, uint32 freq, uint32 duty)
{
    ...
    if(!(MODULE_GTM.CMU.CLK_EN.U & 0x2))
    {
        IfxGtm_Cmu_setClkFrequency(&MODULE_GTM, IfxGtm_Cmu_Clk_7, CMU_CLK_FREQ);
        IfxGtm_Cmu_enableClocks(&MODULE_GTM, IFXGTM_CMU_CLKEN_CLK0);
    }
